...
- **Source guidance (G1):** The default trial tenant does not include Teams. G1
  suggests waiting until the guide's Teams coverage before activating, to preserve the 30 days.
...
**`OnboardingState` lives in two places and they disagree (Lab 03, the device-side verification guide).**
...
- **Source guidance (URBAC roles guide):** The role-creation prerequisites cover who may *create* a role; they say nothing about what the *assignee* needs.
...
- **Source guidance (device-group membership-rule guide):** Create the admin group first, then grant it access to the device group.
...
- **Source guidance (Defender XDR connector guide):** Create a workspace, enable Sentinel, connect data sources; retention and ingestion are the cost levers.
...
- **Source guidance (local-script onboarding guide):** Download the local script, run it elevated on the device, confirm the device appears in the portal.
...
- **Source guidance (URBAC roles guide):** States URBAC is the default model for new tenants — Defender for Endpoint since 2025, Office 365 P2 since July 2026.
...
- **Source guidance (URBAC roles guide):** Data sources are a hard boundary; scoping to a single workload is done by deselecting the others.
...
- **Source guidance (ASR deployment guide):** Deploy via Intune/policy; view results in the ASR report.
...
- **Source guidance (device-group membership-rule guide):** Create the group with a membership rule, set the automation level, grant an Entra group access.
...
- **Source guidance (device-discovery guide):** Standard is default; watch the network scope.
...
- **Source guidance (Defender XDR connector guide):** Install the connector, connect incidents/alerts, optionally stream raw events.
...
| 182 | Device names render truncated in portal display surfaces | `LAB-WIN11-DEFEN` — 15 characters — on **six** surfaces including the **machine-readable Action Center CSV export**, which carries the truncated value as data rather than rendering a display name | The truncation is not a rendering artefact. At minimum it is the device identity MDE holds, whatever the OS hostname says | Lab 22 §8.8; cf. `POS-033` |
| 183 | Deallocating a VM ends the charges for it | Both VMs read `Stopped (deallocated)`; the client VM's public IP is **SKU Standard, Assignment Static**, still associated, and bills continuously for as long as the resource exists — roughly $0.005/hr, ~$3.65/month | Compute stops; the IP meter does not. The mechanism recorded in `docs/evidence-notes/before-allocating-scus.md`, caught from configuration before the invoice rather than inferred from billing afterwards | Lab 22 §9, `POS-094` |
| 184 | Guide: a single combined "Workloads / record types" filter | `Workloads` and `Record Types` render as **two separate fields in different columns**, and a third the guide omits entirely — `Activities - operation names` — is what the search actually ran on | Three filters described as one. The omitted field is the only one that takes the schema string | Lab 23 §4, §7.9 |
| 185 | Field labels describe their contents | The record flyout's `Users` field renders the subject's **object GUID**; the UPN appears lower down as `UserId`, while the grid's `User` column rendered the UPN. `Item` renders a well-known first-party **application ID**. The flyout **never names the raw object** the CSV calls `AuditData` | Three labels that do not describe what they contain, on one pane. `P93-7` confirmed in the strongest form: the portal labels it nothing | Lab 23 §7.8 |
| 186 | Portal surfaces render resolved values | The completed job's `Search Query Information` header renders `, , {9} {10} {11} {12}` — positional format-string indices standing in for the empty optional criteria | Unresolved format tokens on a customer-facing header of a Microsoft security product | Lab 23 §7.10 |
| 187 | Timestamps within one solution share a convention | **Seven renderings of one instant**: header `GMT`; grid `Date (UTC)`; job list `Creation time (UTC-07:00)`; CSV `…Z ` with a trailing space inside the quoted value; raw JSON bare ISO; cmdlet bare US locale; flyout labelled `Date (UTC)` over a bare value. An eighth on the eDiscovery statistics page: `August 11, 2026, 15:35 PM` | All reconcile to the same instant. **The authoritative object is the only one that never states its timezone.** Three of the seven are unlabelled | Lab 23 §7.7 |
| 188 | The friendly Activities list is a way to select an operation | Identical labels appear **across** activity groups and **repeated within** one group, adjacent and indistinguishable. Selecting one drops the group qualifier from the chip | A display vocabulary, not a query key. The field then displays a value that no longer identifies what was chosen | Lab 23 §7.9 |
| 189 | `RecordType` has one representation | Grid `AzureActiveDirectoryStsLogon`; flyout `15`; CSV `15`; cmdlet `AzureActiveDirectoryStsLogon`; payload `Workload` = `AzureActiveDirectory` | Flyout agrees with the export, grid agrees with the cmdlet. **The Azure AD-era vocabulary is a rendered grid column**, not a buried payload value, in 2026 | Lab 23 §7.4 |
| 190 | `Search-UnifiedAuditLog` returns the same records as the portal | Portal export **19**, cmdlet **21**, identical scope. Diffed: the export is a strict subset and both extras are the same records emitted twice — identical `Identity` at two `ResultIndex` positions, `ResultCount 21` | Nineteen distinct records; two emitted twice. No error, no warning. An analyst counting rows overcounts by ten percent unless deduplicating on `Identity` | Lab 23 §7.5, `POS-095` |
| 191 | `UserLoggedIn` records a user logging in | The record examined carried `RequestType: OAuth2:Token`, `UserAgent: Windows-AzureAD-Authentication-Provider/1.0`, and a Microsoft-address-space client IP — a non-interactive token acquisition | Rendered in the grid as `User logged in`, indistinguishable from interactive sign-ins in the same result set. The `4624` principle, measured on a second product | Lab 23 §7.6 |
| 192 | A mid-run result count is a partial answer | `Total results` read `8` at 77.78% and `19` at completion. `Progress` rendered exactly 7/9 — a fixed unit count, not an estimate | Not proportional. Reading the in-progress figure as the result is wrong by more than half. One of three unpopulated-not-empty figures in a single job | Lab 23 §7.3 |
| 193 | Global Administrator can reach every Purview solution | `eDiscovery Manager` held **0 users, 0 security groups** by default; Content Search refused, and the identity is not a member of the system-generated case | Separation of duties by design. A tenant administrator can manage every service and still not read the organisation's mail and documents | Lab 23 §8.1, `POS-096` |
| 194 | An error dialog carries the actionable message | **Two stacked modals.** On top a `Client Error` reading only "Missing required permissions to view case"; beneath it a `Permission Error` carrying the cause and remedy in full | The useless one renders first. A user who dismisses it and navigates away never sees the actionable one | Lab 23 §8.1 |
| 195 | A page that renders its controls has granted access | `Case settings`, `Process manager`, `Summarize this case` and the descriptive text all rendered while every data-bearing element was withheld | Case-scoped action buttons render without the case being readable. Same class as `POS-035`'s `System > Audit`, now measured in a second solution | Lab 23 §8.1 |
| 196 | Removing a role group removes the access it granted | Role group at 0 users on three surfaces; the portal continued to render the case and its searches across two reads fifteen minutes apart. A write attempt returned `you're not a member of this compliance case`, and the Cases list then read `0 items` | **`configured ≠ effective` on the revocation side.** Only attempting an operation revealed the true state; every surface built to answer the access question rendered a state the service did not honour | Lab 23 §8.5, `POS-097` |
| 197 | A blank `Created` column means the object pre-existed | `Case settings` gives `Case created: 8/11/2026 8:51:57 PM +00:00` — three minutes before the first search. The case was **created by the grant**, on first access | A claim made during the lab and falsified within it. Absence in a column is not evidence of age | Lab 23 §8.5, `POS-097` |
| 198 | One product's Copilot integration has one provisioning state | `Draft a query with Copilot` enabled its controls, rendered `Generating KeyQL…` and declined on prompt **content**. `Summarize this search` refused: *"Copilot hasn't been set up yet."* Same session, same identity, same page | Content-level failure and availability refusal, side by side. **Narrows `POS-091`** — "torn down and therefore absent" holds for the surface it described and must not be generalised | Lab 23 §8.6, `POS-091` |
| 199 | Purview auditing is per-user licensing with no consumption charge | The Audit solution renders a **`Pay-as-you-go usage report`** in its own left navigation, and tenant OCR scanning is **charged per use against Microsoft Syntex billing** | Three cost mechanisms in one product: E5-family per-seat entitlements, a pay-as-you-go audit meter, and a Syntex consumption meter | Lab 23 §7.1 |
| 200 | An image-only keyword is not discoverable without OCR | Both visible OCR controls were **unchecked** — tenant scanning greyed behind Syntex billing, case-level `Enable OCR` off — and the PNG **returned**. Its only occurrence of the term was rendered pixels, verified pre-upload as absent from the file bytes and from all PNG text chunks | Text extraction from images happens on a path neither visible OCR setting governs. A pre-explanation from licensing was **falsified** | Lab 23 §8.3, `POS-098` |
| 201 | Accounting for partially indexed items reveals what a search missed | A plain-ASCII file with an unrecognised extension, containing the term in body text, was absent from results, reported as `Partially indexed items 0 (0 B)`, and reported as `Matches from advanced indexing 0` on a rerun where the advanced pass demonstrably ran | **Three surfaces, three zeros, one readable file.** Extension-based exclusion sits outside the partially-indexed model entirely; opting in does not reveal it | Lab 23 §8.4, `POS-098` |
| 202 | The partially-indexed controls are peers on the search screen | `Include partially indexed items` ships **unchecked**; checking it reveals two nested options — one of them advanced indexing — and checking that reveals a third-level runtime warning | The advanced-indexing control cannot be discovered without first checking a box that is off by default | Lab 23 §8.4 |
| 203 | Cost analysis at billing-account scope reports the account's spend | The billing-account scope rendered **`No cost reported during this period`** with `ACTUAL COST` as `--` for August, while the same tenant's subscription scope for the same month reported **$9.09** across six services | A false empty produced by scope selection, on a surface whose empty state names a *period* and not a scope. The zero was read first and would have falsified nothing correctly — the two-read rule is what caught it | 2026-08-12 cost read, `POS-099` |
| 204 | A service that costs nothing is reported as costing nothing | Sentinel emits **daily zero-cost meter rows** (`Free Trial Analysis`, `Free Benefit - M365 Defender Analysis`) throughout the export; Azure Bastion Developer SKU emits **no row at all** for a host created, used and deleted on 2026-08-08 | **Two different zeros in one file.** A measured zero and an absent row are not the same claim, and only the presence of the first makes the second interpretable. Meter-based cost tooling cannot see what emits no meter | 2026-08-12 cost read, `POS-099` |
| 205 | Deallocating both VMs stops the subscription's daily spend | The public IP meter billed **$0.12/day on 29 of 29 days**, while the VM compute meter appears on **9 of 29** — twenty days of IP charge with no VM running | The quantified form of row 183 and `POS-094`: allocation billing proven against VM state rather than inferred from a flat total. $3.40 accrued from one unused address | 2026-08-12 cost read, `POS-094` |
| 206 | A cost forecast projects from observed run-rate | A single non-recurring $4.00 `Provisioned Security Compute Unit` charge on 08-09 produced a forecast of **$3.32 on 08-30** — the model treating one Copilot provisioning event as periodic | Forecast is extrapolation, not commitment. A one-off charge inside a short history distorts the projection for the rest of the month; the $20.57 month-end figure inherits that distortion | 2026-08-12 cost read |
| 207 | `gh run list --commit <sha>` locates the workflow run for that commit | Returns **`no runs found`** for an abbreviated SHA; the run exists and is green under the full 40-character form. Proven by a control pair differing only in SHA length | **Our own verification tooling returning a silent false negative.** The step exists to bind CI confirmation to a specific commit, and the short form defeats it while looking like a legitimate answer — no error, no warning. Every SHA-bound confirm in prior sessions used the short form. Fix: `$(git rev-parse HEAD)` | 2026-08-12, `f99ec8b` |
| 208 | `contains` is the safe, broad fallback when `==` returns nothing | Three operators against `LogonType` on `EntraIdSignInEvents`, one query, 30,801 rows: **`==` → 0**, **`contains` → 30,801 (every row)**, **`has` → 933 (correct)**. Mechanism verified: `contains` is case-insensitive, so case-folding makes `nonInteractiveUser` contain `interactiveUser` as a substring at index 5 — the negation is invisible to the operator | **Both wrong answers are plausible numbers and neither errors.** Zero reads as a quiet tenant; 30,801 reads as a busy one; the truth is between them. A detection rule written with `==` never fires and the same rule with `contains` fires on everything. The published guidance frames `has` as the *faster* operator; here it is the only *correct* one | 2026-08-12, advanced hunting |
| 209 | `union withsource=T A, B, C, D \| summarize by T` reports on every source named | A source with **zero rows contributes no rows**, so it vanishes from the result entirely rather than rendering `Rows = 0`. `CloudAppEvents` was absent from a four-table union; only a separate bare `\| count` established it existed and was empty | **The absence is of a row describing the absence.** Four tables were asked about and three answered; nothing on the surface indicates the fourth was ever in the query. A third instance of the repo's recurring shape — see rows 97 and 202 — and the reason the `CloudAppEvents` finding needed a second query to be conclusive | 2026-08-12, advanced hunting |
| 210 | A product page describes its subscription's state consistently | On 2026-08-14, one page rendered simultaneously: banner 1 **"Your Office 365 E5 expired."** (past tense, informational icon), banner 2 *"Your free trial **will expire** on 8/13/2026 and your service **will end**"*, banner 3 *"This subscription **will be canceled when it expires** on August 13, 2026"*, and `Subscription status:` **`✅ Active`** | **Two tenses and a contradicting status field on one panel.** Banner 1 had also *de-escalated* — red error icon on the expiry date, informational after it — so severity peaked on the date and dropped once it passed. A reader taking any single element as the answer gets a different answer depending which they read | Lab 24 §6.2, `POS-100` |
| 211 | Two banners about one subscription age the same way | Across 2026-08-06 → 08-12 → 08-13 → 08-14, the `Licenses and apps` banner read **`8/12/2026`** on all four dates while the product-page banner moved through **three states** over the same interval: `expires on 8/13/2026` → `expires today` → `expired` | **The difference is a property of the field, not of the data.** One surface is a stale fixed string, the other a relative renderer, and only reading both across the same interval distinguishes them. The stale one is still advising the reader to buy *"so they won't lose access when the trial ends"* after the date it names has passed | Lab 24 §4, §5, §6.2 |
| 212 | A CSV export is a neutral machine-readable rendering of the page | The Microsoft 365 E5 `Renewal or expiration date` field contains **two U+200E left-to-right marks** wrapping the date — `Renews on \u200e9/14/2026\u200e with 1 paid license` — while the Office 365 E5 field, a bare date, contains none. File also carries a UTF-8 BOM. Verified programmatically on three exports across two dates | **Bidirectional formatting controls embedded inside a data field**, invisible in every viewer, present in one row and absent in another of the same column — an internal control within a single exported column. Enough to break a naive date parse or an exact-match comparison, and undetectable by reading the file | Lab 24 §8.4 |
| 213 | `sentinel/mitre-coverage.md` (`ms.date: 06/16/2025`): "By default, both currently active scheduled query and near real-time (NRT) rules are indicated in the coverage matrix" | `Active rules` ships at `4 selected` — `Active scheduled query rules`, `Active NRT query rules`, `Active anomaly query rules`, **`Active custom detection rules`**. Observed 2026-08-14 and again 2026-08-16 | The default matrix population is broader than the documentation describes and **can include Defender XDR custom detections** alongside Sentinel analytics-rule populations. A coverage figure read from the default view is not a Sentinel-only figure, and nothing on the page says so | `docs/evidence-notes/mitre-attack-coverage-matrix.md` |
| 214 | Docs: the Defender pane shows recommended coverage as **a ratio** of active detections and security services (products) out of all recommended detections and services | `Recommended coverage details` renders `Active coverage (7)` with a rule-type breakdown and `Simulated coverage (0)`. No products dimension, no denominator, no ratio | The documented shape and the rendered shape differ. Anyone scripting or reporting against the documented ratio finds a count | `docs/evidence-notes/mitre-attack-coverage-matrix.md` |
| 215 | Intel management empty state: "Once a workspace is hydrated with TI data into the `ThreatIntelligenceIndicator` table, analytics rules match event log data with the TI data" | Data landed in `ThreatIntelIndicators`. `ThreatIntelligenceIndicator` is present, queryable, Analytics tier, 30 days, and returned **zero rows** on 2026-08-14. Current Sentinel TI ingestion no longer targets the legacy table, and this workspace was created after that migration. The same page banners the migration away from it | Onboarding guidance names a destination that current ingestion does not target, on a page that simultaneously announces the migration. An analyst following the empty state writes queries against a table this workspace has no path to populate | Lab 25 §4.2 |
| 216 | `TimeGenerated` is one column with one value | Azure Logs renders `8/14/2026, 10:45:52.126 PM` under a column header explicitly labelled `[UTC]`. Defender surfaces render `Aug 14, 2026 3:45:52 PM` — local, unlabelled, truncated to seconds. Same row, same column, seven hours apart. Observed on four Defender pages | An analyst moving a timestamp between the two portals is seven hours out with **no marker on the Defender side** to indicate local rendering. Precision is also lost silently | `docs/evidence-notes/search-jobs.md` |
| 217 | Hunt Bookmarks empty state: "Select a hunt query from the Microsoft Sentinel hunts page and click 'View query results' … to view the results **in Log Analytics**. Use the check boxes … and click 'Add bookmark'" | `View results` routes to **Advanced hunting** from both the main Queries tab and from inside a hunt. No bookmark control exists there — with a row selected the toolbar offers `Link to incident` and `Take actions`. The hunt's Bookmarks tab has no creation control. Confirmed twice, 36 hours apart, fresh session | The bookmark creation path documented on the page is unreachable by the route the page itself describes. Bookmarks are **not** broken: the same operation succeeds via Azure Logs reached from Sentinel → Search. The fault is routing, in one specific path | Lab 26 §4.7 |
| 218 | Hunting query creation guidance leads with `SigninLogs \| where TimeGenerated >= ago(1h)` | The creation form's own banner: "Do not use fixed time ranges, either directly or in a function, in your query. Otherwise, we cannot show changes in query results over time." Windowing belongs to the page's scope control; the `Results delta` and `Results delta percentage` columns depend on it | A query authored per the guidance saves and runs, and the delta columns are silently meaningless. The form forbids what the guidance instructs | `docs/evidence-notes/hunting-queries.md` |
