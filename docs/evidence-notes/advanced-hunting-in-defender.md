...
Concept, and the most consequential guide in the section for work already
designed.

Two constraints land directly on **Lab 25**:
...
- **In-query time filters change which data is queried.** When a query sets its
  ...
  across the full 30 days the time-range picker is used instead. Lab 25's
...
The source guide's four causes for an empty table in a lab tenant — no activity, not
licensed, outside retention, not onboarded — is the taxonomy `docs/evidence-notes/kql-fundamentals-and-surfaces.md` walked
