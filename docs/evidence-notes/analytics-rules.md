...
> The preceding source material was conceptual and produced no standalone evidence note; its content is
...
thirteen incidents, where Lab 10's MDO incident reads `Queued` (row 39). The actions-and-submissions guide
established that an alert is not an action; for Sentinel rules there is no
action path at all.

**Baseline before either rule: 0 active rules.** A workspace ingesting data with
no detections running — the analytics-rules concept guide's "without analytics rules Sentinel is a data
lake," demonstrated rather than asserted. *(See `docs/evidence-notes/enabling-ueba.md`: that count excludes
...
`SecurityIncident`, both measured non-billable in the entities-and-UEBA guide. Twelve duplicate
