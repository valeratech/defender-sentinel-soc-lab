...
# Creating a Security Copilot capacity

> The source guide the meter starts on. Its real content is not the create flow but
...
**The defaults are the guide.** `Enable Overage Capacity` ships **on**, set to
...
the capacity's deletion (`docs/evidence-notes/tearing-down-copilot-capacity.md`).
...
Whether the M365 E5 **inclusion** path renders this flow differently. The Copilot overview, SCU-capacity and pre-allocation guides recorded the question as open; Lab 20 resolved it for **this** tenant —
