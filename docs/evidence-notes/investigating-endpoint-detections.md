...
> Nominally: open the incident the last walkthrough produced and work it. Actually:
...
Nothing. The whole walkthrough is portal reads at zero cost with both VMs
...
**The cloud adjudicated a file the endpoint ignored** (Lab 22 §8.2). From `docs/evidence-notes/endpoint-prevention-controls.md`, the
...
by sweeping, not by being told. Without the unrelated EICAR test from `docs/evidence-notes/endpoint-prevention-controls.md` there
...
continuously for as long as it exists. The mechanism recorded in `docs/evidence-notes/before-allocating-scus.md`, caught from
...
plus the portal's global toolbar. Capacity was torn down at `docs/evidence-notes/tearing-down-copilot-capacity.md` (Lab 20). The
surface renders normally and the pane is not on it. This is a positive absence
on a working surface, **not** a foreclosed scenario in the shape of `docs/evidence-notes/sample-alerts-skipped.md`.
