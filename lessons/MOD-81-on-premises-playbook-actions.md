...
# Module 81 — Run playbooks on on-premises resources

> A boundary module, and the footnote Lab 19 left open. The template deployed
> there is named `Reset Microsoft Entra ID User Password` — the scope limit is
> in the product name. Module 81 answers the question that name raises: what
...
Nothing, and nothing could be. This is the first module in the project whose
...
1. **No on-premises resource exists to reach.** No local AD, no domain
   controller. The module's premise is what is missing, not the mechanism.
...
The boundary is also softer than the module implies. With Entra Connect and
