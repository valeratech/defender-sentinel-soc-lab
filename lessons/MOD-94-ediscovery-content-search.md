...
# Module 94 — Content search in eDiscovery
...
>
> Also, unavoidably, an RBAC lab — because the module could not begin until a
...
and documents. It is also MOD-92's licensing trap on a second axis —
...
consistent with the MOD-88 capacity teardown, the KeyQL drafter is not gated on
...
module's record: I pre-explained the image control's outcome from licensing
...
narrowed by this module) · Lab 20 (MOD-88 capacity teardown) · Lab 22 (the
negative-then-positive control precedent, and the deliberately-left evidence
precedent) · MOD-92 (entitlement is not assignment, licensing axis) ·
`SANITIZATION.md` §2 (the `LAB-WIN11-DEFEN` alias originates in this module's
audit payloads)
