#!/usr/bin/env bash
# MUTATING (remote). FINAL governed push of main through the L2 native pre-push hook.
# Runs only after 45 (accepted baseline committed) and 50 (L2 verify PASS).
# Expect exactly one U6_RETURN line from the hook before git's own output.
